...
class StockMovement extends \Eloquent{
    ...
    public static function getSourceName($source){
        if($source == StockMovement::SOURCE_SALE)           return trans('mojito.sale');
        if($source == StockMovement::SOURCE_REVO_STOCKS)    return trans('mojito.revoStocks');
        if($source == StockMovement::SOURCE_PURCHASE)       return trans('mojito.purchase');
        if($source == StockMovement::SOURCE_BACK)           return trans('mojito.back');
    }
